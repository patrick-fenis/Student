...
Set up the following route handler in your express app:
...
```javascript
curl -X POST -d name="dragon fruit" localhost:3000/products
```

or

```javascript
curl -X POST -d name="dragon fruit" -d color="pink" localhost:3000/products
```

For each new key/value pair, add a new `-d property="value"` argument

```
curl -X POST -d name="kiwi" -d color="green" -d readyToEat="on" localhost:3000/products
```
...
`extended: true` - has to do with how the data is being parsed (and what kind can be parsed). 
...
```javascript
curl -X POST -d name="dragon fruit" -d color="pink" -d localhost:3000/products
```
